    ...
    document.getElementById('loginForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        try {
          ...
          showLoginError('Server returned invalid response (HTTP ' + res.status + ')');
          return;
        }
        ...
        if (data.access_token) {
          ...
          showLoginError(data.error || data.errcode || 'Login failed (HTTP ' + res.status + ')');
        }
      } catch (err) {
        showLoginError('Connection error: ' + (err.message || err));
      }
    });
